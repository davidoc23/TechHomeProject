...
def test_add_room_valid(client, auth_headers):
    response = client.post('/api/rooms/', json={"name": "Living Room"}, headers=auth_headers)
    assert response.status_code == 201
    data = response.get_json()
    assert data['name'] == "Living Room"
    assert 'id' in data
    db.rooms_collection.delete_one({"_id": ObjectId(data['id'])})
...
def test_update_room_valid(client, auth_headers):
    inserted = db.rooms_collection.insert_one({"name": "Old Room"})
    room_id = str(inserted.inserted_id)
    response = client.put(f'/api/rooms/{room_id}', json={"name": "Updated Room"}, headers=auth_headers)
    assert response.status_code == 200
    data = response.get_json()
    assert data['name'] == "Updated Room"
    db.rooms_collection.delete_one({"_id": ObjectId(room_id)})

def test_update_room_invalid_id(client, auth_headers):
    response = client.put('/api/rooms/000000000000000000000000', json={"name": "Updated"}, headers=auth_headers)
    assert response.status_code in [404, 500]

def test_delete_room_valid(client, auth_headers):
    inserted = db.rooms_collection.insert_one({"name": "Delete Me"})
    room_id = str(inserted.inserted_id)
    response = client.delete(f'/api/rooms/{room_id}', headers=auth_headers)
    assert response.status_code == 200

def test_delete_room_with_devices(client, auth_headers):
    room_id = db.rooms_collection.insert_one({"name": "Has Devices"}).inserted_id
    ...
    db.devices_collection.delete_many({"roomId": room_id})
    db.rooms_collection.delete_one({"_id": room_id})

def test_get_room_devices(client, auth_headers):
    room_id = db.rooms_collection.insert_one({"name": "Room With Device"}).inserted_id
    db.devices_collection.insert_one({
        "name": "Fan",
        "type": "fan",
        "isOn": False,
    ...
    assert devices[0]['name'] == "Fan"
    db.devices_collection.delete_many({"roomId": room_id})
    db.rooms_collection.delete_one({"_id": room_id})

def test_update_room_same_name(client, auth_headers):
    inserted = db.rooms_collection.insert_one({"name": "Duplicate Room"})
    room_id = str(inserted.inserted_id)
    response = client.put(f'/api/rooms/{room_id}', json={"name": "Duplicate Room"}, headers=auth_headers)
    assert response.status_code == 200
    db.rooms_collection.delete_one({"_id": ObjectId(room_id)})
